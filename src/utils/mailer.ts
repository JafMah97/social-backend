...
const resend = new Resend(
  process.env.RESEND_API_KEY || 're_Za6v1HL6_BFaBrUafoSkG97jTNsFrduZq',
)
    ...
    <div style="font-family: Arial, sans-serif; color: #333; background-color: #f8f9fa; padding: 20px;">
      <div style="text-align: center; margin-bottom: 30px;">
        <h1 style="text-align: center; font-family: Arial, sans-serif;">
          <span style="display: inline-block; width: 40px; height: 40px; border-radius: 50%; background: linear-gradient(135deg, #ff69b4, #8a2be2); color: white; font-size: 24px; font-weight: bold; line-height: 40px; text-align: center; vertical-align: middle; margin-right: 0px;">K</span>
      ...
      ${verifyLink ? `<p>You can also <a href="${verifyLink}" style="color: #1e90ff;">click here to verify your email</a> instantly.</p>` : ''}
...
export async function sendPasswordResetLink(email: string, token: string) {
  ...
  await resend.emails.send({
    from: 'Konekta <onboarding@resend.dev>',
    to: email,
